"""
Developed by Michael Fong
WEA+ Logger / Visualizer

How to setup:
python logger.py <port>
Use nohup or screen to run it in the background
nohup.out will have all server responses. 

ps -f -C python
this tells you what python processes are running

pkill -f "python logger.py"
kills the service.

Usability Notes:
around line 400 there are two ip's that you need to change depending on where you're
running the server. 

inputervin.py and getervin.py
these are necessary to get the location information from the CROSSMobile user location
server.
...
most of the others are tables that I used for debugging or seeing the data in other ways
/testalertid
was the main one

Future plans for the Visualizer:
Would like to collect data in a separate database and then view the answers the questions 
in a visualization, thinking either by alertid and responses to certain questions and such.

Also to move the code possibly to a more suitable language such as Ruby on Rails or Nodejs. 
Right now a lot of the code is implemented using python javascript strings, would be cleaner to 
do it differently. 
"""
...
class testalertid:
    def GET(self):
        ...
        #new = m[0]
        t = HTML.Table(header_row=['alertId', 'phoneId', 'geolocation', 'abstractLocation', 'timeReceived', 'timeAcknowledged', 'version', 'answers', 'comment', 'inTarget', 'crossId'])
        ...
        for new in m:
            ...
            t.rows.append([new.alertId, new.phoneId, new.geolocation, new.abstractLocation, readtr, readta, new.version, new.answers, new.comment, new.inTarget, new.crossId])
        htmlcode = str(t)
            #print htmlcode
        return htmlcode#new#("AlertID: "+new.AlertID +" "+ "PhoneID: "+new.PhoneID)
class viewquestions:
    ...
    def POST(self):
       ...
       #print feedbackExists
        ...
        n = db.insert('data', alertId=entry["alertId"], phoneId= entry["phoneId"], geolocation=(str(entry["geolocation"][0])+","+str(entry["geolocation"][1])),
         ...
         question10 =  entry["feedback"]["answers"][9]["question"], feedbackExists = feedbackExists, numQuestions = numQuestions, reset = 0, isWEAPlus = 1, response = 'Evacuate', alerttype = 'Extreme',
...
function getMarkers(){
    //url = 'http://http://192.168.50.21/:8001/fullrefresh?alertId=%(urlAlertId)s';
    url = 'fullrefresh?alertId=1';
...
$(document).ready(function(){
  $("#reset").click(function(){
    ...
    url = 'fullreset?alertId=%(urlAlertId)s';
  ...
  $("#autorefresh").click(function(){
    ...
    if(buttonOn){
    //console.log("refresh");
      refresher = setInterval(function(){getMarkers()}, 3000);
    ...
    def POST(self):
      ...
      for new in entry:
          for x in keys:
              ...
              if new[x] is None:
                new[x] = '0'
          m = db.query("UPDATE data SET geolocation = (" + "'"+str(new['latitude'])+ "," + str(new['longitude'])+"'), crossId = '" + str(new['subscriber']) +"'" + 
...
class polygon:
  ...
  #SETTING UP THE POLYGON
          ...
          alertrow = 12
          #values_list.index(newalert) + 1
          globalAlertId = worksheet.acell("A"+str(alertrow)).value
          newalert = str(globalAlertId)
          urlAlertId = newalert
...
"""
        ...
        m = db.query("SELECT * FROM data WHERE (alertId = "+globalAlertId+")")
        dataone = ""
        datatwo = ""
        centerlat = 0
        centerlong = 0
...
"""
            ...
            currLat.append(float(split[0]))
            currLong.append(float(split[1]))
        ...
        totalLat = float(0)
        totalLong = float(0)
        for x in range(len(currLat)):
          totalLat += currLat[x]
          totalLong += currLong[x] 
        print "totalLat: " + str(totalLat)
        print "totalLong: " + str(totalLong)
        if(isReset== False):
          polygondata +="""
alertpolygon['test'] = {
  center: new google.maps.LatLng(""" + str(totalLat)+ "," + str(totalLong) + """),
};
"""
...
markers = [];"""
        ...
        return removepolygon + removemarkers + dataone + content + datatwo + listener +markerarray
#could optimize by getting polygon last. instead of polygon first and then markers later.        
class fullrefresh:
    def GET(self):
        db = web.database(dbn='mysql', user='root', pw='12345', db='responses')
        user_data = web.input()
